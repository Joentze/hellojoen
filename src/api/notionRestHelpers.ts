...
const TIMELINE_ID = (): string => { return process.env.REACT_APP_NOTION_DB_TIMELINE_ID === undefined ? '' : process.env.REACT_APP_NOTION_DB_TIMELINE_ID }
...
export const getTimelineList = async (): Promise<NotionResponse> => {
  const response = await axios.get(`${URL()}/db/${TIMELINE_ID()}`)
  return response
}
